...
public class Player_orbits : MonoBehaviour
{
    [SerializeField] private AudioClip _moveClip, _loseClip, _pointClip;

    [SerializeField] private GameplayManager_orbits _gm;
    ...
    private void Update()
    {
        if(canClick && Input.GetMouseButtonDown(0))
        {
            ...
            SoundManager_orbits.Instance.PlaySound(_moveClip);
        }
    }
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if(collision.CompareTag("Obstacle"))
        {
            ...
            SoundManager_orbits.Instance.PlaySound(_loseClip);
            _gm.GameEnded();
            return;
        }
        ...
        if(collision.CompareTag("Score"))
        {
            ...
            SoundManager_orbits.Instance.PlaySound(_pointClip);
            _gm.UpdateScore();
            collision.gameObject.GetComponent<Score_orbits>().ScoreAdded();
            return;
        }
    }
